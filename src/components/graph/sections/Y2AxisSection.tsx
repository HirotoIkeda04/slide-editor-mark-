...
interface Y2AxisSectionProps {
  table: TableItem
  config: ChartYAxisConfig
  onConfigChange: (config: ChartYAxisConfig) => void
  usedColumnIndicesFromY1?: Set<number>
}

export const Y2AxisSection = ({
  table,
  config,
  onConfigChange,
  usedColumnIndicesFromY1 = new Set(),
}: Y2AxisSectionProps) => {
  // Y2軸が有効かどうかは系列の有無で判定
  const hasSeries = config.stacks.some(stack => stack.length > 0)
  const [isOpen, setIsOpen] = useState(hasSeries)
  ...
  // 利用可能なカラム（Y1軸で使用されているものも含む）
  const availableColumns = useMemo(() => {
    return numericColumns.filter(col => 
      !usedColumnIndices.has(col.index) && !usedColumnIndicesFromY1.has(col.index)
    )
  }, [numericColumns, usedColumnIndices, usedColumnIndicesFromY1])
  ...
  const addColumnAsNewStack = useCallback((columnIndex: number) => {
    // Y2軸はデフォルトで線グラフ
    const series = createSeries(columnIndex, 'line', usedColors)
  ...
  // 系列のドロップ（スタック内）
  const handleSeriesDrop = useCallback((e: DragEvent, targetStackIndex: number) => {
    ...
    if (source === 'available') {
      ...
      if (seriesId && !isNaN(fromStackIndex) && fromStackIndex !== targetStackIndex) {
        ...
        if (seriesIndex !== -1) {
          ...
          newStacks[targetStackIndex] = [...newStacks[targetStackIndex], series]
  ...
  }, [config, addColumnToStack, onConfigChange])
  ...
  return (
    <div className="graph-panel-section chart-axis-section">
      ...
      >
        ...
        <span className="chart-section-title">Y2軸（右）</span>
      </button>
